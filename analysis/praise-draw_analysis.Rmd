...
Combine and clean.
```{r}
all.raw = full_join(p3.raw,d) # change this later to combine w/ demographics data sheet
...
all.data = all.raw %>%
  ...
  filter(condition == "tryit" | condition == "selective" | condition == "overpraise",
         subID != "PD2_190715_04",
         subID != "PD_190712_03") %>% #didn't do selective teacher drawing
      #   selective_prompt == "anything", overpraise_prompt == "anything") %>%
  mutate(order = ifelse(CB <= 4, "selective_overpraise", "overpraise_selective"),
         effort_chose_selective = explicit_effort == selective_teacher,
         pref_chose_selective = teacher_pref == selective_teacher) %>%
...
strokes.table = strokes.table %>%
    group_by(age_bin) %>%
  mutate(diff = m_selective - m_overpraise,
         more = ifelse(diff > 0, "sp","op"))
strokes.table

strokes.counts = strokes.table %>%
  group_by(age_bin) %>%
  summarise(n_sp = sum(more == "sp"),
            n_op = sum(more == "op"))
strokes.counts
...
id.means = id.table %>%
  gather(teacher, practices, sp, op) %>%
  group_by(teacher) %>%
  summarize(n=n(),m_practices = mean(practices))
...
ggplot() +
  geom_point(data=id.means, aes(x=teacher, y=m_practices, col=teacher, size=4)) +
#  ylim(0,4) +
  geom_jitter(data=id.tidy, aes(x=teacher, y=practices), size=.5, width=0, height=.1) +
    theme_few() +
  ylab("Number of re-dos")
...
ggplot(id.tidy, aes(x=teacher, y=practices)) +
  ...
  geom_point() +
  theme_few()
#  facet_grid(order ~ age)

ggplot(time.tidy, aes(x=teacher, y=time)) +
    geom_line(aes(group = id), size = 0.5) + 
   # geom_jitter(width = .1, height =.1,size = 2)+
  geom_point() +
  theme_few()
...
bw.table = full_join(overpraise.table, selective.table)
...
explicit.table = all.data %>%
  group_by(id) %>%
  summarize(n=n(),
            num_effort_chose_selective = sum(effort_chose_selective)/n,
            num_pref_chose_selective = sum(pref_chose_selective)/n) %>%
  ungroup() %>%
  summarize(effort_chose_selective = sum(num_effort_chose_selective),
            pref_chose_selective = sum(num_pref_chose_selective),
            n = n())
